...
function Dispatch() {
  const { setDisptchedOpen, bgPos, setBgPos, selectedAwbs, setSelectedAwbs } =
    ...
    <>
      <div className="flex flex-col justify-between w-[40vw] gap-4 text-[#18181B] bg-white rounded-xl shadow-xl px-10 py-9 relative">
        ...
        <div className="flex flex-col gap-2">
          ...
          {/* INFO BOX */}
          <div
            className="bg-[#FBF3E0] text-xs text-[#C3B600] rounded-md border border-[#F9F06D] 
          ...
          >
            ...
            />
            <p>
              2-3 working day. Please be informed Pick Up details will be shared
              on WhatsApp.
            </p>
          </div>
        </div>
...
const ManifestActionButtons = ({ setDisptchedOpen, ctaButtonLabel }) => {
  ...
  const handleSubmitManifest = async () => {
    ...
    try {
      ...
      const res = await axios.put(`${server}/portal/manifest`, payload);
      ...
      console.error(
        "Manifest dispatch failed:",
        err.response?.data || err.message,
      );
      alert(
        `Failed to dispatch manifest: ${err.response?.data?.message || err.message}`,
      );
    }
  };
  ...
  <div className="flex items-center border border-[#979797] rounded-lg p-0.5 relative overflow-hidden mt-2">
    <div
      className={`transition-transform ${
        bgPos ? "translate-x-[98.5%]" : ""
      } bg-[var(--primary-color)] w-1/2 h-11 absolute rounded-lg -z-0`}
    ></div>
    <div
      onClick={() => setBgPos(false)}
      className={`flex items-center justify-center cursor-pointer w-1/2 h-11 text-center rounded-lg ${
        bgPos ? "text-[#979797]" : "text-white font-bold"
    ...
    <div
      ...
      className={`flex items-center justify-center cursor-pointer w-1/2 h-11 text-center rounded-lg ${
        bgPos ? "text-white font-bold" : "text-[#979797]"
      } text-xs z-0 transition-colors`}
    >
      Drop at Hub
    </div>
          ...
          {branches.map((branch, index) => (
            ...
            >
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-[#979797]">{branch.code}</span>
              </div>
            </li>
          ))}
        </ul>
