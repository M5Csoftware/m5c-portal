...
const SelectService = ({
  ...
  const [localFilteredServices, setLocalFilteredServices] = useState([]);
  const [isLoadingRates, setIsLoadingRates] = useState(false);
  const [rateError, setRateError] = useState("");

  // ✅ Round up chargeable weight to nearest whole number
  const roundedChargeableWt = chargeableWt ? Math.ceil(chargeableWt) : 0;
  ...
  const extractArrayFromResponse = (response, fieldName) => {
    ...
    if (Array.isArray(response)) {
      console.log(
        `${fieldName}: Response is already an array with ${response.length} items`,
      );
      return response;
    }
    ...
    for (const field of possibleFields) {
      if (response[field] && Array.isArray(response[field])) {
        console.log(
          `${fieldName}: Found array in response.${field} with ${response[field].length} items`,
        );
    ...
    console.error(
      `${fieldName}: Could not find array in response. Response type:`,
      typeof response,
    );
  ...
  useEffect(() => {
    const fetchServicesWithRates = async () => {
      // ✅ Use roundedChargeableWt for validation
      if (!roundedChargeableWt || roundedChargeableWt <= 0) {
      ...
      try {
        ...
        const tariffServices = extractArrayFromResponse(
          tariffRes.data,
          "Tariff Services",
        );
        ...
        console.log(
          `Found ${tariffServices.length} tariff services:`,
          tariffServices,
        );
        ...
        console.log(
          `Found ${commonServices.length} common services:`,
          commonServices,
        );
        ...
        for (let srv of commonServices) {
          ...
          try {
            ...
            if (
              needsZipcode &&
              (!consigneeZipcode || consigneeZipcode.trim().length < 3)
            ) {
              console.warn(
                `Skipping ${srv.service} - zipcode required but not provided`,
              );
              continue;
            }

            // ✅ Use roundedChargeableWt in API params
            const params = new URLSearchParams({
              service: srv.service,
              rateTariff: srv.rateTariff || "",
              chargeableWt: roundedChargeableWt,
              actualWt: actualWt || roundedChargeableWt,
              pcs: pcs || 1,
            ...
            if (needsZipcode && consigneeZipcode) {
              const cleanedZip = consigneeZipcode
                .replace(/\s+/g, "")
                .toUpperCase();
            ...
            if (rateData && rateData.rate && Number(rateData.rate) > 0) {
              ...
              // ✅ Use roundedChargeableWt for amount calculation
              if (rateData.type === "B") {
                basicAmt = rateData.rate * roundedChargeableWt;
              ...
              console.log(
                `Successfully calculated rate for ${srv.service}:`,
                serviceRate,
              );
              finalRates.push(serviceRate);
            } else {
              console.warn(
                `No valid rate returned for ${srv.service}:`,
                rateData,
              );
            }
          } catch (error) {
        ...
        console.log(
          `Final unique services: ${uniqueResults.length}`,
          uniqueResults,
        );
        ...
        if (uniqueResults.length === 0) {
          setRateError(
            "No rates available for the current weight and destination",
          );
        }
      } catch (error) {
        ...
        setRateError(
          error.response?.data?.error ||
            "Failed to fetch service rates. Please try again.",
        );
      } finally {
    ...
    roundedChargeableWt, // ✅ Use roundedChargeableWt in dependency array
    selectedSector,
    selectedDestination,
        ...
        <div className="flex items-center gap-8 px-2 py-6 justify-between w-full">
          ...
          <div className="relative flex items-center w-full">
            ...
            <div className="absolute left-1/2 flex flex-col items-center gap-2 -top-7">
              ...
              />
              <div>
                {/* ✅ Display roundedChargeableWt */}
                <p className="text-[#979797] text-sm text-center">
                  {roundedChargeableWt || "0"}
                </p>
          ...
          {isLoadingRates ? (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-500 mt-2">
                    Please check your inputs or contact support if the issue
                    persists.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
              ...
              <p className="text-xs text-yellow-600 mt-1">
                Please verify your destination, weight, or consignee postal
                code.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex flex-col gap-6">
                    <div className="flex items-center gap-2">
                      ...
                      {(service.isCanadaShipment ||
                        service.isAustraliaShipment) && (
