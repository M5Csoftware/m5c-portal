...
function FilterCustomerSupport({ onApplyFilters }) {
  ...
  useEffect(() => {
    if (ticketsData && Array.isArray(ticketsData)) {
      ...
      const uniqueTicketIds = [
        ...new Set(
          ticketsData
            .map((ticket) => ticket.ticketId || ticket.displayTicketId)
            .filter(Boolean),
        ),
      ];
      setTicketIdOptions(
        uniqueTicketIds.map((id) => ({ value: id, label: id })),
      );

      // Get unique AWB numbers
      const uniqueAwbNumbers = [
        ...new Set(ticketsData.map((ticket) => ticket.awbNo).filter(Boolean)),
      ];
      setAwbNumberOptions(
        uniqueAwbNumbers.map((awb) => ({ value: awb, label: awb })),
      );

      // Get unique sub categories
      const uniqueSubCategories = [
        ...new Set(
          ticketsData.map((ticket) => ticket.subCategory).filter(Boolean),
        ),
      ];
      setSubCategoryOptions(
        uniqueSubCategories.map((cat) => ({ value: cat, label: cat })),
      );

      // Get unique statuses
      const uniqueStatuses = [
        ...new Set(ticketsData.map((ticket) => ticket.status).filter(Boolean)),
      ];
      setStatusOptions(
  ...
  return (
    <div className="relative">
      <div
        className={`transition-all duration-500 ease-in-out ${filterCustomerSupportWindow ? "max-w-[600px]" : "max-w-0 opacity-0"}`}
      >
        <div className="flex justify-between p-6">
          ...
          <button
            className="flex"
            onClick={() => setFilterCustomerSupportWindow(false)}
          >
            <Image
