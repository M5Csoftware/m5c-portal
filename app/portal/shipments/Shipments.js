...
const Shipments = ({ setTotalShipments, searchTerm, onDownloadSetup, onSelectedCountChange }) => {
  const [shipmentsData, setShipmentsData] = useState([]);
  const [selectedShipments, setSelectedShipments] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(15);
  const { setAccountCode, setSelectedAwbs, server, selectedLi } = useContext(GlobalContext);
  ...
  const filteredShipments = useMemo(() => {
    let filtered = shipmentsData.filter((shipment) => {
      // Status filtering based on selectedLi
      ...
      const searchMatch = searchTerm === "" ||
        ...
        shipment.receiverFullName?.toLowerCase().includes(searchTerm.toLowerCase());

      return statusMatch && searchMatch;
    });

    return filtered;
  }, [shipmentsData, selectedLi, searchTerm]);
  ...
  const downloadExcel = () => {
    ...
    const shipmentsToDownload = selectedShipments.length > 0
      ? shipmentsData.filter(shipment => selectedShipments.includes(shipment._id))
      : filteredShipments;
    ...
    const excelData = shipmentsToDownload.map(shipment => ({
      ...
      'Created Date': shipment.createdAt ? new Date(shipment.createdAt).toLocaleDateString() : '',
      ...
      >
        <div className="flex items-center">
          ...
          <span className="ml-4 text-sm">
            Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredShipments.length)} of {filteredShipments.length} shipments
          </span>
        </div>
        <div className="flex items-center">
          <button
